<?php namespace App\Plugins\CustomFields\Http\DataTables;

use App\Module\Base\Http\DataTables\AbstractDataTables;
use App\Plugins\CustomFields\Models\FieldGroup;
use App\Plugins\CustomFields\Repositories\Contracts\FieldGroupContract;
...
class FieldGroupsListDataTable extends AbstractDataTables
{
    ...
    public function run()
    {
        $this->setAjaxUrl(route('custom-fields.index.post'), 'POST');
    ...
    protected function fetch()
    {
        $this->fetch = datatable()->of($this->model)
            ...
            ->addColumn('actions', function ($item) {
                /*Edit link*/
                $editLink = route('custom-fields.field-group.edit.get', ['id' => $item->id]);
                $disableLink = route('custom-fields.field-group.update-status.post', ['id' => $item->id, 'status' => 'disabled']);
                $activeLink = route('custom-fields.field-group.update-status.post', ['id' => $item->id, 'status' => 'activated']);
                $deleteLink = route('custom-fields.field-group.delete', ['id' => $item->id]);
