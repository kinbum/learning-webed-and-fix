<?php namespace App\Module\Users\Http\DataTables;
...
class UsersListDataTable extends AbstractDataTables
{
    ...
    public function run()
    {
        $this->setAjaxUrl(route('users.index.post'), 'POST');
    ...
    protected function fetch()
    {
        $this->fetch = datatable()->of($this->model)
            ...
            ->addColumn('actions', function ($item) {
                /*Edit link*/
                $activeLink = route('users.update-status.post', ['id' => $item->id, 'status' => 'activated']);
                $disableLink = route('users.update-status.post', ['id' => $item->id, 'status' => 'disabled']);
                $deleteLink = route('users.delete.delete', ['id' => $item->id]);
                $forceDelete = route('users.force-delete.delete', ['id' => $item->id]);
                $restoreLink = route('users.restore.post', ['id' => $item->id]);

                /*Buttons*/
                $editBtn = link_to(route('users.edit.get', ['id' => $item->id]), 'Edit', ['class' => 'btn btn-outline green btn-sm']);
