    ...
    @if(isset($menuNodes) && $menuNodes->count())
        <ul role="menu" id="{{ array_get($options, 'id') }}" class="{{ $isChild === true ? array_get($options, 'submenu_class') : array_get($options, 'class') }}">
            @foreach($menuNodes as $node)
                ...
                @endphp
                <li class="menu-item{{ array_get($options, 'item_class') }}{{ in_array((int)$node->id, $childActivatedNodes) ? 'current-parent-menu-item' : '' }}{{ $isMenuItemActive ? 'active' : '' }}{{ $node->children && $node->children->count() > 0 ? 'menu-item-has-children ' . array_ge($options, 'has_sub_class') : '' }}">
                    <a href="{{ $node->url }}" @if($node->target) target="{{ $node->target }}" @endif title="{{ $node->title }}">@if($node->icon_font) <i class="{{ $node->icon_font }}"></i>@endif {{ $node->title }}</a>
                    @include('menu::front._renderer.menu', [
